...
The bwrap profile combines a read-only host root, a fresh `/dev`, and `/proc` from a private PID namespace — commands manage their descendants but cannot see host processes, so procfs magic links cannot bypass the mounts; `workspace-write` adds an ephemeral `/tmp` and a writable workspace bind. A policy that confines reads binds its read allow-list read-only instead of the whole root, so a path outside the boundary is absent from the wrapped process's filesystem rather than merely unreadable, while `/dev` and `/proc` remain the profile's own fresh mounts. The [private-PID note](../../../.agents/notes/implemented/bug-fix/2026-08-06-bwrap-private-pid-namespace.md) records the boundary.

The `@deepseek-ai/node-addon-system/landlock-run` API supplies the platform launcher, functional probe, and grant vocabulary; this provider maps mode to grants only, keeping path resolution and probe parsing with the versioned binary. A policy that confines reads passes its read allow-list as the launcher's read-only grants in place of the whole tree, while the read-write grants stay exactly the mode's writable roots.

The Seatbelt profile is allow-default with `(deny file-write*)` plus write allow-lists derived from the shared `writableRoots` helper, so exactly the mode's promised file effects are governed; every root is canonicalized because Seatbelt matches resolved paths (`/tmp` IS `/private/tmp`). A policy that confines reads adds `(deny file-read*)` followed by one `file-read*` allow-list of the shared read roots plus the writable roots: SBPL lets the last matching rule decide, and `file-write*` does not imply `file-read*`, so a path this call may write stays readable.

The Windows rung keeps one deterministic write SID and standing ACE per workspace, while every live session/workspace pair gets a random private temp directory with a distinct SID and revocable ACE — sessions sharing a workspace share its intended write authority without inheriting one another's temp authority. A fresh provider always chooses a new temp path and SID, so crash residue cannot block or authorize a resumed session. The rung reports `partial` enforcement because the restricted token must retain Everyone and NTFS hard links alias one file object across paths. A policy that confines reads fails closed on this rung: the restricted token governs DACL writes and has no read rule, so `confine()` throws `SANDBOX_UNAVAILABLE` instead of running the command with an unenforced read boundary.
...
- **The Windows rung cannot enforce a read boundary** — the restricted token has no read rule, so a policy that confines reads fails closed there with `SANDBOX_UNAVAILABLE` instead of running the command with unconfined reads.
