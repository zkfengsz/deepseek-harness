...
import { ESCALATION_TARGETS, READ_ESCALATION_TARGET, approveEscalation, readRootsFor, validateEscalationArgs } from '@deepseek-ai/dsh-sandbox'
import type { SandboxPolicyService } from '@deepseek-ai/dsh-sandbox-policy'
import type { ShellRunResult } from '@deepseek-ai/dsh-shell'
import { parseExitStatus } from '@deepseek-ai/dsh-shell'
import { processOutcome } from './background.ts'
import { renderPwshProcessRead, renderPwshResult } from './render.ts'
import type { EscalationTarget, RenderablePwshResult } from './render.ts'
...
function pwshDescription(backgroundEnabled: boolean, escalationTargets: readonly EscalationTarget[]): string {
  ...
  // (escalationTargets non-empty). Every shipped composition pairing tool-pwsh
  ...
  const escalation = base + ' Under the Windows sandbox, read-only pwsh runs in PowerShell ConstrainedLanguage mode, while '
...
/* jscpd:ignore-start -- deliberate mirror of dsh-tool-bash's escalation field text and grant
   application (pwsh-tool-and-executor Agent Note). */
/**
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    name: 'pwsh',
    description: pwshDescription(backgroundEnabled, escalationTargets),
    ...
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: value.kind === 'background'
          ...
          : renderPwshResult(value as RenderablePwshResult, escalationTargets),
      }],
    },
        ...
        const id = jobs.start({
          ...
          run: () => {
            ...
            return {
              ...
              readOutput: () => renderPwshProcessRead(proc.readOutput(), proc.sandbox, escalationTargets),
            }
          },
        })
