...
describe('sandbox escalation through ctx.approval', () => {
  ...
  it('advertises read-anywhere only when the deployment confines reads', async () => {
    ...
    expect(plain.ctx.tools.schemas().find(item => item.name === 'pwsh')!.description).not.toContain('read-anywhere')
    ...
    const confined = await setupSandboxed(false, { confineReads: true, readRoots: ['C:\\data'] })
    expect(enumOf(confined.ctx)).toEqual(['workspace-write', 'danger-full-access', 'read-anywhere'])
    expect(confined.ctx.tools.schemas().find(item => item.name === 'pwsh')!.description).toContain(
  ...
  it('runs the granted read escalation with the boundary lifted and the mode unchanged', async () => {
    ...
    ctx.agents.register(agent)
    const read = {
      command: 'Get-Content C:\\data\\ledger.csv',
    ...
    expect((await call(ctx, 'pwsh', { command: 'Write-Output ok', description: 'ordinary call' }, agent)).isError).toBe(false)
    expect(bash.policies.map(policy => policy.readRoots)).toEqual([['/data']])

    const escalated = await call(ctx, 'pwsh', { ...read, sandbox_permissions: 'read-anywhere' }, agent)
  ...
  it('keeps the read boundary when only a wider mode was granted', async () => {
    ...
    const result = await call(ctx, 'pwsh', {
      command: 'New-Item -ItemType Directory -Force C:\\data\\out',
  ...
  it('refuses read-anywhere when this call has no confined reads, without prompting', async () => {
    ...
    const rejected = await call(unconfined.ctx, 'pwsh', {
      command: 'Get-Content C:\\data\\ledger.csv',
      description: 'read the ledger',
      sandbox_permissions: 'read-anywhere',
      justification: 'the file sits outside the read boundary',
  ...
  it('reports a read denial through the marker and lifts the boundary for the approved retry', async () => {
    ...
    const agent = sandboxAgent('read-only', ctx)
    ctx.agents.register(agent)
    const read = { command: 'Get-Content C:\\elsewhere\\report.csv', description: 'read a file outside the boundary' }

    bash.denied = true
    const denied = await call(ctx, 'pwsh', read, agent)
    ...
    bash.denied = false
    ctx.on('approval/request', () => Promise.resolve<ApprovalOutcome>('allowed-once'))
    const retried = await call(ctx, 'pwsh', {
      ...read,
      sandbox_permissions: 'read-anywhere',
      justification: 'the file sits outside the read boundary',
    }, agent)
