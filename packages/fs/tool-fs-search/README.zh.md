...
使用 `dsh-tool-fs-search` 为模型提供本地工作区中的 `glob` 文件发现与 `grep` 内容搜索。搜索无需在宿主上安装 `rg`，也无需文件系统提供方；结果相对于工作目录，并包含隐藏与忽略文件但排除 VCS 元数据。约束会话读取的部署同样把每次搜索的根目录限制在同一边界内，并允许一次获批的 `read-anywhere` 重试。可配置上限约束内联输出；挂载可选 spill 存储后，达到上限的结果仍可完整恢复。若需读取、写入或编辑文件，请选择同级 `dsh-tool-fs` 包。
...
搜索失败携带本包定义的错误码：`SEARCH_INVALID_PATTERN`（ripgrep 拒绝正则或 glob）、`SEARCH_FAILED`（启动失败、目标不可访问、信号终止或 `--json` 输出格式错误）、`SEARCH_RAW_OUTPUT_OVERFLOW`（原始输出超过上限）与 `SEARCH_ABORTED`（协作式超时或调用方取消）与 `SEARCH_DENIED`（搜索根位于调用会话的数据边界之外，或部署无法证明它不在边界之外）。退出 0 表示成功且有结果，退出 1 表示成功的空搜索；模型参数错误仍是普通工具参数错误。

### 会话读取边界

当部署设置 `sandboxPolicy.confineReads` 时，每次搜索都会在启动进程之前检查根目录：该根必须是 `readRootsFor` 为调用会话派生的某个根（其工作区、部署配置的读取根，以及进程运行所需的平台根），比较使用 `ctx.fs` 暴露的包含关系测试。未指定根目录时默认使用会话工作区，它始终位于边界内。被拒绝的搜索返回 `[sandbox: file read denied outside this session's data boundary]`，并附带指出 `sandbox_permissions: "read-anywhere"` 的提示；两个工具仅在读取受约束时声明该参数与 `justification`，获批的重试只放行那一次搜索。未组合文件系统的部署无法为受约束会话证明包含关系，因此会失败关闭而不是搜索宿主。部署事实来自 `ctx.sandboxPolicy`；无 agent（智能体）的调用从不受约束，因此 harness 自身的搜索照常工作。
...
- **读取边界只检查搜索根，而不检查 ripgrep 打开的每个文件**——位于边界内的根目录若其子树包含指向外部的符号链接，不会按文件重新检查，因为 ripgrep 通过自身进程读取而非 `ctx.fs`；harness 自带的读取工具会对打开的每个文件做规范化。
